...
import io.sharedstreets.barefoot.topology.Path;
...
/**
 * Route in a {@link RoadEdge} network that consists of a start and end {@link RoadPoint} and sequence
 * of {@link RoadEdge}s.
 */
public class Route extends Path<RoadEdge> {
     ...
     * @param roads Sequence of {@link RoadEdge}s that make the route.
     */
    public Route(RoadPoint source, RoadPoint target, List<RoadEdge> roads) {
        super(source, target, roads);
    }

    /**
     * Gets size of the route, i.e. the number of {@link RoadEdge}s that make the route.
     *
     * @return Number of the {@link RoadEdge}s that make the route.
     */
    public int size() {
        return path().size();
    }
    ...
    /**
     * Gets {@link RoadEdge} by index of the sequence of {@link RoadEdge} objects.
     *
     * @param index Index of the road to be returned from the sequence of roads.
     * @return RoadEdge at the given index.
     */
    public RoadEdge get(int index) {
    ...
    @Override
    public boolean add(Path<RoadEdge> other) {
        time = null;
        length = null;
    ...
    public static Route fromJSON(JSONObject json, RoadMap map) throws JSONException {
        LinkedList<RoadEdge> roads = new LinkedList<>();
        ...
        for (int j = 0; j < jsonroads.length(); ++j) {
            ...
            roads.add(RoadEdge.fromJSON(jsonroad, map));
        }
